"""В этом модуле реализованы встроенные процедуры языка Scheme."""
...
try:
    ...
    print("предупреждение: не в силах импортировать модуль turtle.", file=sys.stderr)

class SchemeError(Exception):
    """Сообщение, сообщающее об ошибке в Scheme-программе."""

########################
# Встроенные процедуры #
########################

# Список триплетов (ИМЯ, PYTHON-функция, ВНУТРЕННЕЕ-ИМЯ). Наполняется в
# builtin и используется в scheme.create_global_frame.
BUILTINS = []

def builtin(*names):
    """Аннотация для преобразования Python-функции в BuiltinProcedure."""
    def add(fn):
        for name in names:
...
def check_type(val, predicate, k, name):
    """Возвращает VAL. Поднимает SchemeError если не PREDICATE(VAL),
    используя "аргумент K с именем NAME" для описания проблемного значения."""
    if not predicate(val):
        msg = "аргумент {0} с именем {1} имеет неверный тип ({2})"
...
def scheme_truep(val):
    """Все значения в Scheme истинны, за исключением False."""
    return val is not False

def scheme_falsep(val):
    """Только False является ложной величиной в Scheme."""
    return val is False
...
def scheme_cdr_stream(x):
    check_type(x, lambda x: scheme_pairp(x) and scheme_promisep(x.second),\
               0, 'cdr-stream')
...
@builtin("list?")
def scheme_listp(x):
    """Проверяет, что x форматный список. Предполагает отсутствие циклов."""
    while x is not nil:
...
def _check_nums(*vals):
    """Проверяет, что все аргументы в VALS являются числами."""
    for i, v in enumerate(vals):
        if not scheme_numberp(v):
            msg = "операнд {0} ({1}) не число"
            raise SchemeError(msg.format(i, v))

def _arith(fn, init, vals):
    """Применяет функцию FN ко всем числовым значениям в VALS.
    INIT — значение, если VALS пуст.
    Возвращает результат в виде Scheme-значения.
    """
    _check_nums(*vals)
    s = init
    for val in vals:
...
def number_fn(module, name, fallback=None):
    """Создание встроенной Scheme-процедуры, принимающей числа,
    реализация которой на Python находятся в модуле MODULE с именем NAME.
    """
...
# Добавление ряда функций из модуля math во встроенные процедуры Scheme
...
builtin("log2")(number_fn(math, "log2", lambda x: math.log(x, 2)))  # совместимость с Python 2
...
##################################################
## Turtle-графика (только в этом интерпретаторе) #
##################################################
...
def tscheme_forward(n):
    """Перемещает черепашку вперед на расстояние в N единиц
...
def tscheme_backward(n):
    """Перемещает черепашку назад на расстояние в N единиц
    в текущем направлении."""
    _check_nums(n)
    _tscheme_prep()
    turtle.backward(n)
...
@builtin("left", "lt")
def tscheme_left(n):
    """Вращение черепашки на N градусов против часовой стрелки."""
    _check_nums(n)
    _tscheme_prep()
    turtle.left(n)
...
@builtin("right", "rt")
def tscheme_right(n):
    """Вращение черепашки на N градусов по часовой стрелке."""
    _check_nums(n)
    _tscheme_prep()
    turtle.right(n)
...
def tscheme_circle(r, extent=None):
    ...
    Рисует окружность с центром на расстоянии R единиц слева от черепашки
    (или вправо, если N отрицательный). Если EXTENT не None, то рисуется
    дуга на EXTENT градусов от круга.
    Если R < 0 окружность рисуется по часовой стрелке,
    если R > 0 -- окружность рисуется против часовой стрелки.
    """
    if extent is None:
        _check_nums(r)
    else:
...
def tscheme_setposition(x, y):
    """Устанавливает координаты черепашки в (X,Y), направление не меняется."""
    _check_nums(x, y)
    _tscheme_prep()
...
def tscheme_setheading(h):
    """Устанавливает направление в H градусов по часовой стрелке
    от севера (верха)."""
    _check_nums(h)
    _tscheme_prep()
    turtle.setheading(h)
...
@builtin("penup", "pu")
def tscheme_penup():
    """Поднимает перо, черепашка перестаёт рисовать."""
    _tscheme_prep()
    turtle.penup()

@builtin("pendown", "pd")
def tscheme_pendown():
    """Опускает перо, черепашка начинает рисовать."""
    _tscheme_prep()
    turtle.pendown()
...
@builtin("clear")
def tscheme_clear():
    """Удалить нарисованное, параметры черепашки не меняются."""
    _tscheme_prep()
    turtle.clear()

@builtin("color")
def tscheme_color(c):
    """Устанавливает цвет пера. С — строка, например '"red"'
    или '"#ffc0c0"' (шестнадцатеричные значениё красного, зеленого и синего).
    """
    _tscheme_prep()
...
def tscheme_rgb(red, green, blue):
    """Создает цвет из дробных значений 0..1 красного, зеленого и синего."""
    colors = (red, green, blue)
    for x in colors:
        if x < 0 or x > 1:
            raise SchemeError("Недопустимая интенсивность цвета " + repl_str(colors))
    scaled = tuple(int(x*255) for x in colors)
    return '"#%02x%02x%02x"' % scaled

@builtin("begin_fill")
def tscheme_begin_fill():
    """Начало последовательности перемещений для закраски фигуры."""
    _tscheme_prep()
    turtle.begin_fill()

@builtin("end_fill")
def tscheme_end_fill():
    """Конец последовательности перемещений для закраски фигуры."""
    _tscheme_prep()
    turtle.end_fill()
...
def tscheme_exitonclick():
    """Ожидает клик в области окна и закрывает его."""
    global _turtle_screen_on
    if _turtle_screen_on:
        print("Для выхода закрой окно или кликни по нему.")
        turtle.exitonclick()
        _turtle_screen_on = False

@builtin("speed")
def tscheme_speed(s):
    """Устанавливает скорость перемещений черепашки S (целое в интервале
    0-10, 0 — без анимации (линии рисуются мгновенно), 1-10
...
def tscheme_pixel(x, y, c):
    """Рисует закрашенный прямоугольник (по-умолчанию 1 пиксель)
    в координате (x, y) цветом c."""
